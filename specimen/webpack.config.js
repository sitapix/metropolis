...
// webpack 4 and file-loader hash with md4, which OpenSSL 3 does not provide, so
// every build on Node 17 or later dies with ERR_OSSL_EVP_UNSUPPORTED. The hashes
// only name output files, so any algorithm will do. Doing it here rather than
// through NODE_OPTIONS=--openssl-legacy-provider keeps the fix working on the
// older Node versions that reject that flag.
const createHash = crypto.createHash;
crypto.createHash = (algorithm, ...rest) =>
	createHash(algorithm === "md4" ? "sha256" : algorithm, ...rest);
